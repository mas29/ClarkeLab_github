...
# Function to plot 
plot <- function(df, confidence_intervals, phenotypic_marker_name, phenotypic_marker_units, y_axis_limits, compound, target, pathway) {
  ggplot() +
    ...
    ggtitle(paste(phenotypic_marker_name," Levels for ",compound," Over Time\nTarget: ", target, "\nPathway: ", pathway, sep="")) +
...
shinyServer(function(input, output) {
  ...
  output$sparklines <- renderPlot({
    ...
    if (input$marker == "Sytox Green") {
      plot(sytoxG_data_no_NC.sub, confidence_intervals_SG, "Sytox Green", "(green cells / mm^2)", y_limits_SG, input$compound, sytoxG_data_no_NC.sub$Targets[1], sytoxG_data_no_NC.sub$Pathway[1])
    }
