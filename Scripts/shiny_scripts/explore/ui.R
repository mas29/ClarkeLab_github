...
compounds <- sort(unique(data_wide$Compound[which(data_wide$Pathway != "NegControl")]))
compound_list <- as.list(setNames(compounds, compounds))

# Get list of targets
targets <- sort(unique(data_wide$Target.class..11Mar15.[which(data_wide$Pathway != "NegControl")]))
