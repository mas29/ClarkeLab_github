...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#Load XLConnect library:
library(XLConnect, lib.loc = "C:/Users/Dave/Documents/R/win-library/2.15")
...
reconfigureCompoundNames <- function() {
	cmpNameKey <- loadWorkbook("1833Key.xlsx")
	tempList <- list()
...
getDescriptions<- function(){
	...
	descriptionKey <- read.xls("C:/Users/Dave/Documents/SFU job/Lab - muscle signaling/Dixon - myocyte expts/Dave&Scott_C2C12Data_Sep14/L1700-Selleck-Bioactive-Compound-Library-1836cpd.xlsx",sheet=2)
...
getRawData <- function(){
	...
	wb <- loadWorkbook("dc140908 c2c12 diff tun+1833b.xlsx")
	...
	for(h in 1:numPhenotypicMarkers){
		...
		for(i in 1:5){
			...
			for(j in 1:16){
				...
				for(k in 1:24){
					tempCol <- data.frame(frame[,startCol])			
					tempCol = t(tempCol)
...
write.csv(toXL,file = "dc140910 c2c12 diff tun+1833 reorg.csv")
