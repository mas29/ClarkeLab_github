...
#param selleck_bioactive_compound_lib -- selleck bioactive compound library
read_data <- function(plate_file_names, plate_sheet_names, raw_data_start_row, raw_data_start_col, compounds_key_file_name, compounds_key_sheet_names, 
                      num_plates, phenotypic_marker_name, time_elapsed, selleck_bioactive_compound_lib) {
  merged_data <- data.frame()
  for (i in 1:num_plates) {
    #read in raw data for plate
    raw_data <- read.xlsx(plate_file_names[i], sheetName=plate_sheet_names[i], stringsAsFactors=F)
    ...
    compounds <- as.vector(t(read.xlsx(compounds_key_file_name, sheetName=compounds_key_sheet_names[i], stringsAsFactors=F)))
    ...
    if (nrow(merged_data) == 0) {
      merged_data <- trimmed_data[1:nrow(trimmed_data), 1:ncol(trimmed_data)]
    }
    else {
      merged_data <- cbind(merged_data, trimmed_data[1:nrow(trimmed_data), 1:ncol(trimmed_data)])
    }
  }
  ...
  #merge with selleck cheminformatic info !!!!!!!!! new code !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  merged_data <- merge(merged_data, selleck_bioactive_compound_lib, all.x=TRUE, by.x="row.names",by.y="compound")
  return(merged_data)
}
...
#read data
confluency_all_plates_compounds_vs_features <- read_data(plates_raw_data_file_names, confluency_plates_raw_data_sheet_names, 
                                                         raw_data_start_row, raw_data_start_col, compound_key_file_name, compound_key_sheet_names, 
                                                         5, "confluency", time_elapsed, selleck_bioactive_compound_lib)
sytoxG_all_plates_compounds_vs_features <- read_data(plates_raw_data_file_names, sytoxG_plates_raw_data_sheet_names, 
                                                     raw_data_start_row, raw_data_start_col, compound_key_file_name, compound_key_sheet_names, 
                                                     5, "sytoxG", time_elapsed, selleck_bioactive_compound_lib)

temp <- read_data(plates_raw_data_file_names, confluency_plates_raw_data_sheet_names, 
                     raw_data_start_row, raw_data_start_col, compound_key_file_name, compound_key_sheet_names, 
                     5, "confluency", time_elapsed, selleck_bioactive_compound_lib)
#add metrics
...
#!!!!!!!!!!!!!!!!! restructure data properly, include selleck data, substitute spaces in compound names in selleck with dots !!!!
